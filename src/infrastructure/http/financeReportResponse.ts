...
export async function financeReportResponse(request: Request, defaultFormat: "json" | "csv" = "json") {
  try {
    const ctx = await resolveApiActor(request);
    await requirePermission(ctx, "reports:export");
    const { searchParams } = new URL(request.url);
    const typeParam = searchParams.get("type") ?? "finance_dashboard";
    if (!REPORT_TYPES.includes(typeParam as never)) {
      return Response.json({ error: "Unsupported report type", code: "INVALID_REPORT_TYPE" }, { status: 400 });
    }
    const reportType = typeParam as ReportExportSourceModel["reportType"];
    const filters = parseFinanceFilters(searchParams);
    const service = await getFinanceReportingReadService();
    const format = searchParams.get("format") ?? defaultFormat;
    if (format !== "json" && format !== "csv") {
      return Response.json({ error: "Unsupported format", code: "INVALID_REPORT_FORMAT" }, { status: 400 });
    }
