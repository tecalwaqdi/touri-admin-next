...
export class WifDriverReviewRequestBindings implements DriverReviewRequestBindingPort {
  ...
  constructor(
    ...
    workflow: string = "reviewDriverApplicationV2",
  ) {
  ...
  async bind(key: string, fingerprint: string): Promise<void> {
    ...
    try {
      ...
      const response = await this.fetcher(`${COLLECTION_URL}?documentId=${key}`, {
        ...
        body: JSON.stringify({ fields: { fingerprint: { stringValue: fingerprint }, createdAt: { timestampValue: new Date().toISOString() }, workflow: { stringValue: this.workflow } } }),
      });
      ...
      if (stored.fields?.fingerprint?.stringValue !== fingerprint || stored.fields?.workflow?.stringValue !== this.workflow) throw new CanonicalDriverReviewError("IDEMPOTENCY_CONFLICT", 409);
    } catch (error) {
