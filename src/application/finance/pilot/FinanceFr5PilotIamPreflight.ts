/**
 * FR5 Settlement Execution pilot — check-only IAM + ADC principal preflight.
...
export type FinanceFr5PilotIamPreflightResult =
  | {
      ok: true;
      status: "IAM_PREFLIGHT_PASS";
      mutationsPerformed: 0;
      projectId: typeof FINANCE_FR5_EXPECTED_PROJECT_ID;
      ...
      requiredCapabilities: typeof FINANCE_FR5_REQUIRED_OPERATOR_IAM_PERMISSIONS;
...
export async function runFinanceFr5PilotIamPreflight(input?: {
  projectId?: string;
...
}): Promise<FinanceFr5PilotIamPreflightResult> {
  const projectId = input?.projectId?.trim() || FINANCE_FR5_EXPECTED_PROJECT_ID;
  const expectedPrincipal =
    input?.expectedPrincipal ?? FINANCE_FR5_EXPECTED_ADC_PRINCIPAL;
  const expectedAdcPrincipal = FINANCE_FR5_EXPECTED_ADC_PRINCIPAL;
  ...
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS?.trim()) {
    return {
      ...
      requiredCapabilities: FINANCE_FR5_REQUIRED_OPERATOR_IAM_PERMISSIONS,
      grantedPermissions: [],
      missingPermissions: [...FINANCE_FR5_REQUIRED_OPERATOR_IAM_PERMISSIONS],
  ...
  try {
    ...
    if (!offlineInjected) {
      ...
      if (creds.projectId !== FINANCE_FR5_EXPECTED_PROJECT_ID) {
        return {
          ok: false,
          status: "IAM_PREFLIGHT_FAILED",
          mutationsPerformed: 0,
    ...
    } else if (projectId !== FINANCE_FR5_EXPECTED_PROJECT_ID) {
      return {
        ok: false,
        status: "IAM_PREFLIGHT_FAILED",
        mutationsPerformed: 0,
    ...
    const grantedRaw = await tester.testIamPermissions({
      ...
      permissions: FINANCE_FR5_REQUIRED_OPERATOR_IAM_PERMISSIONS,
    });
    const missing = FINANCE_FR5_REQUIRED_OPERATOR_IAM_PERMISSIONS.filter(
      (p) => !grantedRaw.includes(p),
    );
    const granted = FINANCE_FR5_REQUIRED_OPERATOR_IAM_PERMISSIONS.filter((p) =>
