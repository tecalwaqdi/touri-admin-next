/**
 * FR1 pilot apply — check-only IAM + ADC principal preflight.
...
export type FinanceFr1PilotIamPreflightResult =
  | {
      ok: true;
      status: "IAM_PREFLIGHT_PASS";
      mutationsPerformed: 0;
      projectId: typeof FINANCE_FR1_EXPECTED_PROJECT_ID;
      ...
      requiredCapabilities: typeof FINANCE_FR1_REQUIRED_OPERATOR_IAM_PERMISSIONS;
...
export async function runFinanceFr1PilotIamPreflight(input?: {
  projectId?: string;
...
}): Promise<FinanceFr1PilotIamPreflightResult> {
  const projectId = input?.projectId?.trim() || FINANCE_FR1_EXPECTED_PROJECT_ID;
  const expectedPrincipal =
    input?.expectedPrincipal ?? FINANCE_FR1_EXPECTED_ADC_PRINCIPAL;
  const expectedAdcPrincipal = FINANCE_FR1_EXPECTED_ADC_PRINCIPAL;
  ...
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS?.trim()) {
    return {
      ...
      requiredCapabilities: FINANCE_FR1_REQUIRED_OPERATOR_IAM_PERMISSIONS,
      grantedPermissions: [],
      missingPermissions: [...FINANCE_FR1_REQUIRED_OPERATOR_IAM_PERMISSIONS],
  ...
  try {
    // Offline unit tests inject both resolver + tester — skip live ADC network.
    ...
    if (!offlineInjected) {
      ...
      if (creds.projectId !== FINANCE_FR1_EXPECTED_PROJECT_ID) {
        return {
          ok: false,
          status: "IAM_PREFLIGHT_FAILED",
          mutationsPerformed: 0,
    ...
    } else if (projectId !== FINANCE_FR1_EXPECTED_PROJECT_ID) {
      return {
        ok: false,
        status: "IAM_PREFLIGHT_FAILED",
        mutationsPerformed: 0,
    ...
    const grantedRaw = await tester.testIamPermissions({
      ...
      permissions: FINANCE_FR1_REQUIRED_OPERATOR_IAM_PERMISSIONS,
    });
    const missing = FINANCE_FR1_REQUIRED_OPERATOR_IAM_PERMISSIONS.filter(
      (p) => !grantedRaw.includes(p),
    );
    const granted = FINANCE_FR1_REQUIRED_OPERATOR_IAM_PERMISSIONS.filter((p) =>
