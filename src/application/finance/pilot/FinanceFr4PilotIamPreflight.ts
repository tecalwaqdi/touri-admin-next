/**
 * FR4 Settlement Approval pilot — check-only IAM + ADC principal preflight.
 * Reuses FR1 authorized_user tokeninfo principal resolver pattern.
 * Exact permissions for the 4-write approval path. No Auth/Firestore mutation.
 */
...
export type FinanceFr4PilotIamPreflightResult =
  | {
      ok: true;
      status: "IAM_PREFLIGHT_PASS";
      mutationsPerformed: 0;
      projectId: typeof FINANCE_FR4_EXPECTED_PROJECT_ID;
      ...
      requiredCapabilities: typeof FINANCE_FR4_REQUIRED_OPERATOR_IAM_PERMISSIONS;
...
export async function runFinanceFr4PilotIamPreflight(input?: {
  projectId?: string;
...
}): Promise<FinanceFr4PilotIamPreflightResult> {
  const projectId = input?.projectId?.trim() || FINANCE_FR4_EXPECTED_PROJECT_ID;
  const expectedPrincipal =
    input?.expectedPrincipal ?? FINANCE_FR4_EXPECTED_ADC_PRINCIPAL;
  const expectedAdcPrincipal = FINANCE_FR4_EXPECTED_ADC_PRINCIPAL;
  ...
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS?.trim()) {
    return {
      ...
      requiredCapabilities: FINANCE_FR4_REQUIRED_OPERATOR_IAM_PERMISSIONS,
      grantedPermissions: [],
      missingPermissions: [...FINANCE_FR4_REQUIRED_OPERATOR_IAM_PERMISSIONS],
  ...
  try {
    ...
    if (!offlineInjected) {
      ...
      if (creds.projectId !== FINANCE_FR4_EXPECTED_PROJECT_ID) {
        return {
          ok: false,
          status: "IAM_PREFLIGHT_FAILED",
          mutationsPerformed: 0,
    ...
    } else if (projectId !== FINANCE_FR4_EXPECTED_PROJECT_ID) {
      return {
        ok: false,
        status: "IAM_PREFLIGHT_FAILED",
        mutationsPerformed: 0,
    ...
    const grantedRaw = await tester.testIamPermissions({
      ...
      permissions: FINANCE_FR4_REQUIRED_OPERATOR_IAM_PERMISSIONS,
    });
    const missing = FINANCE_FR4_REQUIRED_OPERATOR_IAM_PERMISSIONS.filter(
      (p) => !grantedRaw.includes(p),
    );
    const granted = FINANCE_FR4_REQUIRED_OPERATOR_IAM_PERMISSIONS.filter((p) =>
