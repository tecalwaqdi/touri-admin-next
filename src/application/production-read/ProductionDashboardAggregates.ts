...
export async function computeProductionDashboardAggregates(
  ...
  const completedTrips = tripTruncated
    ? tripScan
    : finalizeAggregateCount({
        count: tripCompleted,
        truncated: false,
  ...
  const cancelledTrips = tripTruncated
    ? tripScan
    : finalizeAggregateCount({
        count: tripCancelled,
        truncated: false,
  ...
  const activeTrips = tripTruncated
    ? tripScan
    : finalizeAggregateCount({
        count: tripActive,
        truncated: false,
  ...
  let driverTruncated = false;
  {
    ...
    while (driverPages < DASHBOARD_AGGREGATE_MAX_PAGES) {
      ...
      cursor = page.nextCursor;
      if (driverPages >= DASHBOARD_AGGREGATE_MAX_PAGES) driverTruncated = true;
    }
  }
  const activeDrivers = finalizeAggregateCount({
    count: driverActive,
    truncated: driverTruncated,
    pagesScanned: driverPages,
    excludedQaCount: driverExcluded,
  });
  const pendingDrivers = finalizeAggregateCount({
    count: driverPending,
    truncated: driverTruncated,
    pagesScanned: driverPages,
    excludedQaCount: driverExcluded,
  ...
  const customers = finalizeAggregateCount({
    count: customerCount,
  ...
  const activeAgents = finalizeAggregateCount({
    count: agentCount,
  ...
  const partners = await scanCatalog({
    fetch: async (cursor) => {
  ...
  const guides = await scanCatalog({
    fetch: async (cursor) => {
  ...
  const fleet = await scanCatalog({
    fetch: async (cursor) => {
      const page = await listFleetCompanies(runtime.client, { limit, cursor });
      return {
        items: page.items
          .filter(
            (i) =>
  ...
  const landmarks = await scanCatalog({
    fetch: async (cursor) => {
  ...
  let supportOpen: DashboardAggregateScanResult;
  try {
    ...
    supportOpen = finalizeAggregateCount({
      count,
  ...
  const kpiAccuracy: DashboardKpiAccuracyMap = {
    totalTrips: tripScan.meta,
    completedTrips: completedTrips.meta,
    cancelledTrips: cancelledTrips.meta,
    activeTrips: activeTrips.meta,
    activeDrivers: activeDrivers.meta,
    customers: customers.meta,
    pendingDrivers: pendingDrivers.meta,
  ...
  return {
    totalTrips: tripScan.value,
    completedTrips: completedTrips.value,
    cancelledTrips: cancelledTrips.value,
    activeTrips: activeTrips.value,
    activeDrivers: activeDrivers.value,
    customers: customers.value,
    pendingDrivers: pendingDrivers.value,
    ...
    sampleIncludesPilotOrTest: includeTest
      ? sampleIncludesPilotOrTest(classifiable)
      : false,
