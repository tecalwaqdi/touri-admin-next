...
import { listProductionCustomersApi } from "@/application/production-read/ProductionOperationalApiReads";
...
export async function GET(request: Request) {
  ...
  if (productionReadPathActive()) {
    try {
      ...
      const result = await listProductionCustomersApi(ctx, {
  ...
  try {
    ...
    const result = await getRepositories().customers.list({
      page,
      pageSize,
      search,
      countryId,
