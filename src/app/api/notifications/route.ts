...
import {
  listProductionAdminNotifications,
  NotificationSourceUnavailableError,
} from "@/application/production-read/NotificationReadService";

/**
 * GET /api/notifications — admin panel notifications (drivers:read or users:manage).
 */
export async function GET(request: Request) {
  try {
    ...
    if (productionReadPathActive()) {
      try {
        const result = await listProductionAdminNotifications(ctx);
        return jsonWithIds(
          {
            items: result.items,
        ...
        if (error instanceof NotificationSourceUnavailableError) {
          return jsonWithIds(
            {
              items: [],
              total: 0,
