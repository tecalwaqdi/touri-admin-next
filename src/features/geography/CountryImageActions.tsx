...
export function CountryImageActions({
  ...
  const run = async (
    ...
    if (action === "replace_country_image") {
      if (!file) {
        setError(t("error"));
        return;
      }
    ...
    try {
      const res = await apiFetch(
        `/api/storage/countries/${encodeURIComponent(countryId)}/images`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "idempotency-key": `country-img-${action}-${countryId}-${Date.now()}`,
          },
          body: JSON.stringify({
            action,
            slotOrIndex: "0",
      ...
      >
        {preview ? (
          ...
          <img
            ...
            data-testid="country-image-local-preview"
          />
        ) : (
        ...
        <input
          ...
          className="hidden"
          data-testid="country-image-file"
          onChange={(e) => {
            ...
            if (file) {
              ...
              void run("replace_country_image", file);
            }
          }}
        />
        <div className="flex flex-wrap gap-1">
          <button
            ...
            disabled={pending}
            data-testid="country-image-replace"
          ...
          {slotPresent ? (
            <button
              ...
              disabled={pending}
              data-testid="country-image-archive"
