...
/**
 * Region single-image replace / archive — Legacy cities.img (regions SoT).
...
export function RegionImageActions({
  ...
  const run = async (
    ...
    if (action === "replace_region_image") {
      if (!file) {
        setError(t("error"));
        return;
      }
    ...
    try {
      const res = await apiFetch(
        `/api/storage/regions/${encodeURIComponent(regionId)}/images`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "idempotency-key": `region-img-${action}-${regionId}-${Date.now()}`,
          },
          body: JSON.stringify({
            action,
            slotOrIndex: "0",
      ...
      >
        {preview ? (
          ...
          <img
            ...
            data-testid="region-image-local-preview"
          />
        ) : (
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (file) {
              ...
              void run("replace_region_image", file);
            }
          }}
        />
        <div className="flex flex-wrap gap-1">
          <button
            ...
            disabled={pending}
            data-testid="region-image-replace"
          ...
          {slotPresent ? (
            <button
              ...
              disabled={pending}
              data-testid="region-image-archive"
