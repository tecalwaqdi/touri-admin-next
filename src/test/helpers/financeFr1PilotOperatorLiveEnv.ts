/**
 * FR1 Finance Pilot — scoped live operator env (independent Finance write gate).
 *
 ...
 * Preserve list (global) only restores FINANCE_FR1_PILOT_APPLY / REGISTRY_PILOT / token —
 * never write-enabling flags (except scoped live re-apply here when armed).
 *
...
export const FINANCE_FR1_PILOT_OPERATOR_LIVE_GATE_KEYS = [
...
export type FinanceFr1PilotOperatorLiveGateKey =
  (typeof FINANCE_FR1_PILOT_OPERATOR_LIVE_GATE_KEYS)[number];

export type FinanceFr1PilotOperatorLiveGateCapture = {
  readonly [K in FinanceFr1PilotOperatorLiveGateKey]?: string;
};

export function captureFinanceFr1PilotOperatorLiveGates(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr1PilotOperatorLiveGateCapture {
  const out: Record<string, string> = {};
  for (const key of FINANCE_FR1_PILOT_OPERATOR_LIVE_GATE_KEYS) {
    const v = env[key];
    if (v !== undefined && v !== "") out[key] = v;
  }
  return out as FinanceFr1PilotOperatorLiveGateCapture;
}

/**
 * Construct FR1 live Finance WRITE process.env BEFORE loadEnv().
 * Call ONLY from finance-fr1-pilot-apply.test.ts when armed.
 *
 * Preserves FIREBASE_ID_TOKEN. ADC only. Independent FINANCE_WRITE_ENABLED.
 * Does not enable GLOBAL/PRODUCTION/Driver/Agent/Customer write switches.
 */
export function applyFinanceFr1PilotLiveWriteEnvironment(input: {
  capturedGates: FinanceFr1PilotOperatorLiveGateCapture;
  ...
  env.EXPECTED_PROJECT_ID =
    env.EXPECTED_PROJECT_ID?.trim() || FINANCE_FR1_EXPECTED_PROJECT_ID;
  env.GOOGLE_CLOUD_PROJECT =
    env.GOOGLE_CLOUD_PROJECT?.trim() || FINANCE_FR1_EXPECTED_PROJECT_ID;

  env.FINANCE_FR1_PILOT_APPLY = env.FINANCE_FR1_PILOT_APPLY?.trim() || "1";
  env.FINANCE_FR1_REGISTRY_PILOT = env.FINANCE_FR1_REGISTRY_PILOT?.trim() || "1";
...
export function loadFinanceFr1PilotLiveWriteEnv(): AppEnvConfig {
  return loadEnv();
}

/**
 * FirebaseAdminFactory Auth path refuses write flags (WRITE_FLAG_DENY).
 * Verified-actor resolution uses a write-disabled view of the same production env.
 */
export function envForFinanceFr1VerifiedActorResolution(
  env: AppEnvConfig,
): AppEnvConfig {
  return {
    ...env,
...
export function readFinanceFr1PilotOperatorGatesFromEnv(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr1OperatorGateEnv {
  return {
