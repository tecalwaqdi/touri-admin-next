/**
 * FR2 Settlement V2 Pilot — scoped live operator env (independent Finance write gate).
 *
...
import { FINANCE_FR2_EXPECTED_PROJECT_ID } from "@/application/finance/pilot/FinanceFr2PilotConstants";
import type { FinanceFr2OperatorGateEnv } from "@/application/finance/pilot/FinanceFr2PilotGates";
import type { OperatorHarnessEnvMap } from "@/test/helpers/operatorHarnessEnvPreservation";

export const FINANCE_FR2_PILOT_OPERATOR_LIVE_GATE_KEYS = [
...
export type FinanceFr2PilotOperatorLiveGateKey =
  (typeof FINANCE_FR2_PILOT_OPERATOR_LIVE_GATE_KEYS)[number];

export type FinanceFr2PilotOperatorLiveGateCapture = {
  readonly [K in FinanceFr2PilotOperatorLiveGateKey]?: string;
};

export function captureFinanceFr2PilotOperatorLiveGates(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr2PilotOperatorLiveGateCapture {
  const out: Record<string, string> = {};
  for (const key of FINANCE_FR2_PILOT_OPERATOR_LIVE_GATE_KEYS) {
    const v = env[key];
    if (v !== undefined && v !== "") out[key] = v;
  }
  return out as FinanceFr2PilotOperatorLiveGateCapture;
}

/**
 * Construct FR2 live Finance WRITE process.env BEFORE loadEnv().
 * Call ONLY from finance-fr2-settlement-pilot-apply.test.ts when armed.
 */
export function applyFinanceFr2PilotLiveWriteEnvironment(input: {
  capturedGates: FinanceFr2PilotOperatorLiveGateCapture;
  ...
  env.EXPECTED_PROJECT_ID =
    env.EXPECTED_PROJECT_ID?.trim() || FINANCE_FR2_EXPECTED_PROJECT_ID;
  env.GOOGLE_CLOUD_PROJECT =
    env.GOOGLE_CLOUD_PROJECT?.trim() || FINANCE_FR2_EXPECTED_PROJECT_ID;

  env.FINANCE_FR2_SETTLEMENT_PILOT_APPLY =
    env.FINANCE_FR2_SETTLEMENT_PILOT_APPLY?.trim() || "1";
  ...
  env.SOURCE = "fr1_snapshot";
  // Ensure FR1 live arm is not concurrently set.
...
export function loadFinanceFr2PilotLiveWriteEnv(): AppEnvConfig {
  return loadEnv();
}

export function envForFinanceFr2VerifiedActorResolution(
  env: AppEnvConfig,
): AppEnvConfig {
  return {
    ...env,
...
export function readFinanceFr2PilotOperatorGatesFromEnv(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr2OperatorGateEnv {
  return {
