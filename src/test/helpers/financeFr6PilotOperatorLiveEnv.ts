/**
 * FR6 Adjustment Pilot — scoped live operator env (independent Finance write gate).
 *
 ...
 * Same Production bootstrap pattern as FR4/FR5 before loadEnv().
 */
...
import { FINANCE_FR6_EXPECTED_PROJECT_ID } from "@/application/finance/pilot/FinanceFr6PilotConstants";
import type { FinanceFr6OperatorGateEnv } from "@/application/finance/pilot/FinanceFr6PilotGates";
import type { OperatorHarnessEnvMap } from "@/test/helpers/operatorHarnessEnvPreservation";

export const FINANCE_FR6_PILOT_OPERATOR_LIVE_GATE_KEYS = [
...
export type FinanceFr6PilotOperatorLiveGateKey =
  (typeof FINANCE_FR6_PILOT_OPERATOR_LIVE_GATE_KEYS)[number];

export type FinanceFr6PilotOperatorLiveGateCapture = {
  readonly [K in FinanceFr6PilotOperatorLiveGateKey]?: string;
};

export function captureFinanceFr6PilotOperatorLiveGates(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr6PilotOperatorLiveGateCapture {
  const out: Record<string, string> = {};
  for (const key of FINANCE_FR6_PILOT_OPERATOR_LIVE_GATE_KEYS) {
    const v = env[key];
    if (v !== undefined && v !== "") out[key] = v;
  }
  return out as FinanceFr6PilotOperatorLiveGateCapture;
}

/**
 * Construct FR6 live Finance WRITE process.env BEFORE loadEnv().
 * Call ONLY from finance-fr6-adjustment-pilot-apply.test.ts when armed.
 */
export function applyFinanceFr6PilotLiveWriteEnvironment(input: {
  capturedGates: FinanceFr6PilotOperatorLiveGateCapture;
  ...
  env.EXPECTED_PROJECT_ID =
    env.EXPECTED_PROJECT_ID?.trim() || FINANCE_FR6_EXPECTED_PROJECT_ID;
  env.GOOGLE_CLOUD_PROJECT =
    env.GOOGLE_CLOUD_PROJECT?.trim() || FINANCE_FR6_EXPECTED_PROJECT_ID;

  env.FINANCE_FR6_ADJUSTMENT_PILOT_APPLY =
    env.FINANCE_FR6_ADJUSTMENT_PILOT_APPLY?.trim() || "1";
  ...
  env.SOURCE = "fr5_settlement_settled";
...
export function loadFinanceFr6PilotLiveWriteEnv(): AppEnvConfig {
  return loadEnv();
}

export function envForFinanceFr6VerifiedActorResolution(
  env: AppEnvConfig,
): AppEnvConfig {
  return {
    ...env,
...
export function readFinanceFr6PilotOperatorGatesFromEnv(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr6OperatorGateEnv {
  return {
