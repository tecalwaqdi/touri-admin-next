/**
 * FR4 Settlement Approval Pilot — scoped live operator env (independent Finance write gate).
 *
...
import { FINANCE_FR4_EXPECTED_PROJECT_ID } from "@/application/finance/pilot/FinanceFr4PilotConstants";
import type { FinanceFr4OperatorGateEnv } from "@/application/finance/pilot/FinanceFr4PilotGates";
import type { OperatorHarnessEnvMap } from "@/test/helpers/operatorHarnessEnvPreservation";

export const FINANCE_FR4_PILOT_OPERATOR_LIVE_GATE_KEYS = [
...
export type FinanceFr4PilotOperatorLiveGateKey =
  (typeof FINANCE_FR4_PILOT_OPERATOR_LIVE_GATE_KEYS)[number];

export type FinanceFr4PilotOperatorLiveGateCapture = {
  readonly [K in FinanceFr4PilotOperatorLiveGateKey]?: string;
};

export function captureFinanceFr4PilotOperatorLiveGates(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr4PilotOperatorLiveGateCapture {
  const out: Record<string, string> = {};
  for (const key of FINANCE_FR4_PILOT_OPERATOR_LIVE_GATE_KEYS) {
    const v = env[key];
    if (v !== undefined && v !== "") out[key] = v;
  }
  return out as FinanceFr4PilotOperatorLiveGateCapture;
}

/**
 * Construct FR4 live Finance WRITE process.env BEFORE loadEnv().
 * Call ONLY from finance-fr4-settlement-approval-pilot-apply.test.ts when armed.
 */
export function applyFinanceFr4PilotLiveWriteEnvironment(input: {
  capturedGates: FinanceFr4PilotOperatorLiveGateCapture;
  ...
  env.EXPECTED_PROJECT_ID =
    env.EXPECTED_PROJECT_ID?.trim() || FINANCE_FR4_EXPECTED_PROJECT_ID;
  env.GOOGLE_CLOUD_PROJECT =
    env.GOOGLE_CLOUD_PROJECT?.trim() || FINANCE_FR4_EXPECTED_PROJECT_ID;

  env.FINANCE_FR4_SETTLEMENT_APPROVAL_PILOT_APPLY =
    env.FINANCE_FR4_SETTLEMENT_APPROVAL_PILOT_APPLY?.trim() || "1";
  ...
  env.SOURCE = "fr2_settlement_draft";
  // Ensure FR1/FR2/FR3 live arms are not concurrently set.
...
export function loadFinanceFr4PilotLiveWriteEnv(): AppEnvConfig {
  return loadEnv();
}

export function envForFinanceFr4VerifiedActorResolution(
  env: AppEnvConfig,
): AppEnvConfig {
  return {
    ...env,
...
export function readFinanceFr4PilotOperatorGatesFromEnv(
  env: OperatorHarnessEnvMap = process.env,
): FinanceFr4OperatorGateEnv {
  return {
    FINANCE_FR4_SETTLEMENT_APPROVAL_PILOT_APPLY:
      env.FINANCE_FR4_SETTLEMENT_APPROVAL_PILOT_APPLY,
