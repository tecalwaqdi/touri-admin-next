#!/usr/bin/env node
/**
 * Atomic Agent Production write pilot (operator-assisted).
 *
 * Auth precedence: same as Driver pilot (see driver-pilot-auth.mjs).
 *
 * Auth / preflight only (no gate arming, no mutation):
 *   AUTH_PREFLIGHT_ONLY=1 FINAL_LIVE_EMAIL=… FINAL_LIVE_PASSWORD=… node scripts/run-agent-production-pilot.mjs
 *
 * Dry gate cycle only (no Agent mutation):
 *   DRY_GATE_CYCLE=1 FINAL_LIVE_EMAIL=… FINAL_LIVE_PASSWORD=… node scripts/run-agent-production-pilot.mjs
 *
 * Negative probe only (arm AGENT, prove Driver 403 canonical, no Agent mutation):
 *   PILOT_NEGATIVE_PROBE_ONLY=1 FINAL_LIVE_EMAIL=… FINAL_LIVE_PASSWORD=… node scripts/run-agent-production-pilot.mjs
 *
 * Lifecycle: PREFLIGHT 40/40 disarmed → ARM GLOBAL+PRODUCTION+AGENT → deploy READY →
 * alias match → LIVE verify → (optional mutation + concurrency) → disarm → LIVE false.
 *
 * Fixture: `.local/write-pilots/agent-fixture.json` (agentIdA, optional agentIdB same country).
 * Artifact: `.local/write-pilots/02-agent.json`
 */
...
const ARTIFACT_PATH = join(OUT_DIR, "02-agent.json");
const WIF_PROOF_PATH = join(OUT_DIR, "00-wif-iam-cloud.json");
const PILOT_NOTE = "Approved Admin Next Agent production write pilot";
const DRIVER_FIXTURE_PATH = join(OUT_DIR, "driver-fixture.json");
...
function agentLooksSynthetic(agent, fixture) {
  if (!agent || typeof agent !== "object") return false;
  if (truthyFlag(agent.synthetic) || truthyFlag(agent.isTest) || truthyFlag(agent.is_test)) {
    return true;
  }
  if (truthyFlag(agent.qaFixture) || truthyFlag(agent.qa_fixture)) return true;
  if (fixture?.synthetic === true) return true;
  if (Array.isArray(fixture?.qaMarkers) && fixture.qaMarkers.length > 0) return true;
  return false;
}

function agentStatusOf(agent) {
  if (!agent || typeof agent !== "object") return null;
  return agent.status || agent.agentStatus || agent.state || null;
}
...
function verifyWifResolvable() {
  ...
  if (!hasOpsSa || !hasWif) {
    throw new Error(
      `WIF_NOT_RESOLVABLE: OPS_WRITE_SA=${hasOpsSa} WIF_PROVIDER=${hasWif}`,
    );
  }
  if (!unlockPresent) {
    throw new Error(
...
async function main() {
  ...
  if (negativeProbeOnly) {
    log(
      "PILOT_NEGATIVE_PROBE_ONLY=1 · arm AGENT only · Driver must be 403 canonical · no Agent mutation",
    );
  }
  ...
  try {
    ...
    if (!idToken) {
      // PILOT_NEGATIVE_PROBE_ONLY requires authenticated Driver 403 isolation —
    ...
    if (!existsSync(FIXTURE_PATH)) {
      report.blocker = "BLOCKED_FIXTURE_MISSING: .local/write-pilots/agent-fixture.json";
      throw new Error(report.blocker);
    }
    const fixture = JSON.parse(readFileSync(FIXTURE_PATH, "utf8"));
    const agentIdA = String(fixture.agentIdA || fixture.agentId || "").trim();
    const agentIdB = String(fixture.agentIdB || "").trim();
    const countryId = String(fixture.countryId || fixture.country || "").trim();
    const expectedState = String(fixture.expectedState || "inactive").trim();
    if (!agentIdA) {
      report.blocker = "BLOCKED_AGENT_FIXTURE_INCOMPLETE: agentIdA required";
      throw new Error(report.blocker);
    }

    if (idToken) {
      const before = await requestJson(`/api/agents/${encodeURIComponent(agentIdA)}`, {
        token: idToken,
      });
      if (before.httpStatus !== 200) {
        if (dryGateCycle && before.httpStatus === 401) {
          ...
          report.fixture = {
            agentIdA,
            agentIdB: agentIdB || null,
            countryId: countryId || null,
            qaSynthetic: fixture.synthetic === true,
            agentStatus: expectedState,
            liveConfirm: "DEFERRED_AUTH",
          };
          report.beforeState = expectedState;
        } else {
          throw new Error(
        ...
        const beforeStatus = agentStatusOf(before.body);
        if (beforeStatus && beforeStatus !== expectedState) {
          throw new Error(`LIVE_FIXTURE_UNEXPECTED_STATE: ${beforeStatus}`);
        }
        if (!agentLooksSynthetic(before.body, fixture)) {
          throw new Error("LIVE_FIXTURE_NOT_SYNTHETIC_QA");
        }
        report.fixture = {
          agentIdA,
          agentIdB: agentIdB || null,
          countryId: countryId || before.body?.countryId || null,
          qaSynthetic: true,
          agentStatus: beforeStatus || expectedState,
          liveConfirm: "PASS",
        };
        report.beforeState = beforeStatus || expectedState;
      }
      ...
      report.beforeState = expectedState;
      report.notes.push(
        "fixture live confirm deferred — file attests inactive synthetic agents; no mutation in dry cycle",
      );
    } else {
    ...
    verifyWifResolvable();
    log("WIF ops-writer resolvable · unlock code present · LIVE false");

    // ---- ARM CONFIG only 3 gates ----
    log("arming CONFIG GLOBAL+PRODUCTION+AGENT only…");
    // Mark armed BEFORE first env mutation so finally always disarms on partial arm.
    gatesWereArmed = true;
    report.gatesArmed = true;
    const armedConfig = armAgentGatesOnly();
    report.configGatesArmed = {
      ...
      AGENT_WRITE_ENABLED: armedConfig.map.AGENT_WRITE_ENABLED,
    };
    log(
      `CONFIG_VALUE armed · GLOBAL=${armedConfig.map.GLOBAL_PRODUCTION_WRITE_ENABLED} PRODUCTION=${armedConfig.map.PRODUCTION_WRITE_ENABLED} AGENT=${armedConfig.map.AGENT_WRITE_ENABLED}`,
    );
    ...
    // Prove Driver write remains blocked (domain gate) while Agent is armed.
    if (idToken) {
      let driverProbe = await requestJson("/api/drivers/probe/approve", {
        method: "POST",
        token: idToken,
        body: {},
      });
      if (!isCanonicalDomainDisabled403(driverProbe) && existsSync(DRIVER_FIXTURE_PATH)) {
        const driverUid = String(
          JSON.parse(readFileSync(DRIVER_FIXTURE_PATH, "utf8")).uid || "",
        ).trim();
        if (driverUid) {
          driverProbe = await requestJson(
            `/api/drivers/${encodeURIComponent(driverUid)}/needs_changes`,
            {
              method: "POST",
              token: idToken,
              body: {
              ...
              headers: { "idempotency-key": `pilot-driver-neg-${randomUUID()}` },
            },
          );
        }
      }
      const driverBlocked = isCanonicalDomainDisabled403(driverProbe);
      if (!driverBlocked) {
        report.driverNegativeProbe = {
          httpStatus: driverProbe.httpStatus,
          code: driverProbe.code,
          blocked: false,
          requiredCanonical403: true,
        };
        report.blocker = `DRIVER_NEGATIVE_PROBE_FAILED: HTTP ${driverProbe.httpStatus} code=${driverProbe.code} (want 403 PRODUCTION_WRITE_DISABLED|RESOURCE_WRITE_DISABLED)`;
        throw new Error(report.blocker);
      }
      report.driverNegativeProbe = {
        httpStatus: driverProbe.httpStatus,
      ...
      log(
        `driver negative probe blocked · HTTP ${driverProbe.httpStatus} code=${driverProbe.code}`,
      );
    } else {
      const driverUnauth = await requestJson("/api/drivers/probe/approve", {
        method: "POST",
        body: {},
      });
      const okUnauth =
        driverUnauth.httpStatus === 401 || isWriteBlockedStatus(driverUnauth);
      report.driverNegativeProbe = {
        httpStatus: driverUnauth.httpStatus,
        code: driverUnauth.code,
        blocked: okUnauth,
        probe: "unauth_dry",
      };
      if (!okUnauth) {
        report.blocker = `DRIVER_UNAUTH_PROBE_FAILED: HTTP ${driverUnauth.httpStatus}`;
        throw new Error(report.blocker);
      }
      log(
        `driver unauth dry probe · HTTP ${driverUnauth.httpStatus} (no auth token)`,
      );
    }
    ...
    if (dryGateCycle) {
      report.notes.push(
        negativeProbeOnly
          ? "PILOT_NEGATIVE_PROBE_ONLY=1 — Driver 403 canonical proven; skipping Agent mutation"
          : "DRY_GATE_CYCLE=1 — skipping Agent mutation; proving arm LIVE true then disarm",
      );
      report.status = "PASS";
      report.expectedPilotMutations = 0;
      ...
      const unauth = await requestJson(
        `/api/agents/${encodeURIComponent(agentIdA)}/activate`,
        {
          method: "POST",
          body: { expectedCurrentState: expectedState },
      ...
      const idemKey = `agent-prod-pilot-activate-${randomUUID()}`;
      log(`POST activate ${agentIdA}…`);
      ...
      report.writeResult = {
        ...
        toState: write.body?.write?.toState ?? agentStatusOf(write.body),
      ...
      const writeOk =
        ...
        (write.body?.write?.toState === "active" ||
          agentStatusOf(write.body) === "active");
      if (!writeOk) {
        report.blocker = `WRITE_FAILED: HTTP ${write.httpStatus} code=${write.code}`;
        throw new Error(report.blocker);
      }

      report.afterState = "active";
      ...
      const after = await requestJson(`/api/agents/${encodeURIComponent(agentIdA)}`, {
        token: idToken,
      });
      const afterStatus = agentStatusOf(after.body);
      if (afterStatus !== "active") {
        throw new Error(`POST_WRITE_STATE_MISMATCH: ${afterStatus}`);
      }
      if (!agentLooksSynthetic(after.body, fixture)) {
      ...
      const replayOk =
        ...
        (replay.body?.write?.status === "idempotent_replay" ||
          agentStatusOf(replay.body) === "active");
      report.idempotency = {
        httpStatus: replay.httpStatus,
        status: replay.body?.write?.status ?? null,
        pass: Boolean(replayOk && afterStatus === "active"),
      };
      ...
      const stale = await requestJson(
        `/api/agents/${encodeURIComponent(agentIdA)}/activate`,
        {
          ...
          body: { expectedCurrentState: expectedState },
          headers: {
            "idempotency-key": `agent-prod-pilot-stale-${randomUUID()}`,
          },
        },
      );
      const staleOk =
      ...
      report.concurrency = {
        ...
        code: stale.code,
        pass: staleOk,
        mode: "stale_expected_state",
      };
      if (!staleOk) {
        throw new Error(
          `STALE_STATE_FAILED: HTTP ${stale.httpStatus} code=${stale.code}`,
        );
      }

      if (agentIdB) {
        const [raceA, raceB] = await Promise.all([
          requestJson(`/api/agents/${encodeURIComponent(agentIdA)}/activate`, {
            method: "POST",
            token: idToken,
            body: { expectedCurrentState: "inactive" },
            headers: {
              "idempotency-key": `agent-concurrent-a-${randomUUID()}`,
            },
          }),
          requestJson(`/api/agents/${encodeURIComponent(agentIdB)}/activate`, {
            method: "POST",
            token: idToken,
            body: { expectedCurrentState: "inactive" },
            headers: {
              "idempotency-key": `agent-concurrent-b-${randomUUID()}`,
            },
          }),
        ]);
        const successCount = [raceA, raceB].filter(
          (r) => r.httpStatus >= 200 && r.httpStatus < 300,
        ).length;
        const blockedCount = [raceA, raceB].filter(
          (r) =>
            r.httpStatus === 409 ||
            String(r.code || r.body?.code || "").includes("COUNTRY") ||
            String(r.code || r.body?.code || "").includes("AGENT"),
        ).length;
        const concurrentPass =
          successCount <= 1 && successCount + blockedCount >= 1;
        report.concurrency = {
          ...report.concurrency,
          concurrentPair: { agentIdA, agentIdB, countryId: countryId || null },
          raceA: { httpStatus: raceA.httpStatus, code: raceA.code },
          raceB: { httpStatus: raceB.httpStatus, code: raceB.code },
          pass: concurrentPass,
          mode: "same_country_pair",
        };
        if (!concurrentPass) {
          report.blocker = `BLOCKED_CONCURRENT_INVARIANT: success=${successCount} blocked=${blockedCount}`;
          throw new Error(report.blocker);
        }
      } else {
        report.notes.push(
          "BLOCKED_CONCURRENT_PAIR: agentIdB missing — skipped same-country concurrent activate",
        );
      }
      ...
      report.legalRestore = {
        attempted: false,
        reason:
          "Operator may deactivate synthetic agent post-pilot; no automatic rollback in harness",
        finalState: "active",
      };
      report.finalFixtureState = "active";
    ...
    try {
      if (gatesWereArmed && keepPassGatesArmed) {
        ...
        if (idToken) {
          ...
          const fixtureAgentId = existsSync(FIXTURE_PATH)
            ? String(
                JSON.parse(readFileSync(FIXTURE_PATH, "utf8")).agentIdA ||
                  JSON.parse(readFileSync(FIXTURE_PATH, "utf8")).agentId ||
                  "__pilot_probe__",
              )
            : "__pilot_probe__";
          const trap403 = await requestJson("/api/drivers/probe/approve", {
            method: "POST",
            token: idToken,
            body: {},
          });
          const agent403 = await requestJson(
            `/api/agents/${encodeURIComponent(fixtureAgentId)}/activate`,
            {
              method: "POST",
              token: idToken,
              body: { expectedCurrentState: "inactive" },
          ...
          const agentBlocked =
            isWriteBlockedStatus(agent403) || isWriteBlockedStatus(trap403);
          report.notes.push(
            `postDisarm agentBlocked=${agentBlocked} trap=${trap403.httpStatus}/${trap403.code} agent=${agent403.httpStatus}/${agent403.code}`,
          );
          if (!agentBlocked) {
            report.status = "FAIL";
            report.blocker =
              report.blocker ||
              `POST_DISARM_AGENT_NOT_403: HTTP ${agent403.httpStatus}`;
            exitCode = 1;
          }
    ...
    const artifact = {
      ...
      agentFixtureId: existsSync(FIXTURE_PATH)
        ? String(
            JSON.parse(readFileSync(FIXTURE_PATH, "utf8")).agentIdA ||
              JSON.parse(readFileSync(FIXTURE_PATH, "utf8")).agentId ||
              "",
      ...
      driverNegativeProbe: report.driverNegativeProbe,
      rbac: report.rbac,
      scope: report.scope,
      idor: report.idor,
    ...
    console.log("==================================================");
    print("AGENT PRODUCTION PILOT:", report.status);
    console.log("");
    ...
    print(
      "FIXTURE:",
      report.fixture
        ? `PASS agentIdA qa_synthetic ${report.fixture.agentStatus ?? report.beforeState}`
    ...
    print(
      ...
      report.pilotExecuted || gatesWereArmed || report.dryGateCycle
        ? "GLOBAL+PRODUCTION+AGENT (session)"
        : "NO",
    );
    print(
      "CONFIG_VALUE ARMED:",
      report.configGatesArmed
        ? `GLOBAL=${report.configGatesArmed.GLOBAL_PRODUCTION_WRITE_ENABLED} PRODUCTION=${report.configGatesArmed.PRODUCTION_WRITE_ENABLED} AGENT=${report.configGatesArmed.AGENT_WRITE_ENABLED}`
        : "n/a",
    );
    print(
    ...
    print("UNRELATED GATES OFF:", report.driverNegativeProbe ? "YES" : "n/a");
    print(
      "DRIVER NEGATIVE PROBE:",
      report.driverNegativeProbe
        ? `${report.driverNegativeProbe.blocked ? "PASS" : "FAIL"} HTTP ${report.driverNegativeProbe.httpStatus} ${report.driverNegativeProbe.code}`
        : "n/a",
    );
    console.log("");
    print(
    ...
    print("FINAL AGENT GATE:", g.AGENT_WRITE_ENABLED || "unknown");
    print(
      "FINAL OTHER GATES:",
    ...
    print(
      ...
      report.status === "PASS"
        ? report.authPreflightOnly
          ? "Auth preflight PASS. Operator real pilot: DRY_GATE_CYCLE=0 FINAL_LIVE_EMAIL=… FINAL_LIVE_PASSWORD=… node scripts/run-agent-production-pilot.mjs"
          : report.dryGateCycle
            ? "Dry gate cycle PASS. Operator mutation command: DRY_GATE_CYCLE=0 FINAL_LIVE_EMAIL=… FINAL_LIVE_PASSWORD=… node scripts/run-agent-production-pilot.mjs"
            : "Agent pilot PASS — continue next domain pilot from runbook."
...
main().catch((err) => {
  console.error(
    `[agent-pilot] FATAL: ${sanitizeMessage(
